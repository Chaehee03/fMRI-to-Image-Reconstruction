...
try:
    ...
    from dalle2_pytorch.dalle2_pytorch import (
        RotaryEmbedding, CausalTransformer, SinusoidalPosEmb, MLP, Rearrange,
        repeat, rearrange, prob_mask_like, LayerNorm, RelPosBias, Attention, FeedForward
    )
    _HAS_DALLE2 = True
except ImportError:
    DiffusionPrior = None
    DiffusionPriorNetworkConfig = None
    _HAS_DALLE2 = False
...
class BrainNetwork(nn.Module):
    def __init__(self, out_dim=768, in_dim=15724, clip_size=768, h=4096, n_blocks=4, norm_type='ln', act_first=False,
                 use_projector=True, use_input_layernorm=True):
        super().__init__()
        self.use_input_layernorm = use_input_layernorm
        if use_input_layernorm:
            self.input_norm = nn.LayerNorm(in_dim, elementwise_affine=True)
...
class BrainDiffusionPriorOld(DiffusionPrior):
    ...
    def from_pretrained(net_kwargs={}, prior_kwargs={}, voxel2clip_path=None, ckpt_dir='./checkpoints'):
        ...
        diffusion_prior = BrainDiffusionPriorOld(net=diffusion_prior_network, clip=None, **kwargs).to(
            torch.device('cpu'))
