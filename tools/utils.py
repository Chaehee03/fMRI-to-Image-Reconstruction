...
def mixco_nce(preds, perm, betas, select=None):
    # preds: [B, B] logits
    B = preds.shape[0]
    device = preds.device

    # ---- 강제 정규화 / 검증 ----
    assert preds.ndim == 2 and preds.shape[1] == B, f"preds must be [B,B], got {preds.shape}"

    perm = perm.reshape(-1).to(device=device, dtype=torch.long)
    betas = betas.reshape(-1).to(device=device, dtype=preds.dtype)

    if select is not None:
        select = select.to(device=device)
        assert select.shape[0] == B, f"select len {select.shape[0]} != B {B}"

    # 범위 확인 (디버깅에 매우 도움)
    if torch.any((perm < 0) | (perm >= B)):
        # 필요한 경우 아래처럼 강제 보정 가능(권장: 먼저 assert로 잡고 원인 해결)
        # perm = perm % B
        bad_min = int(perm.min().item())
        bad_max = int(perm.max().item())
        raise ValueError(f"perm out of range: [{bad_min}, {bad_max}] but B={B}")

    rows = torch.arange(B, device=device)
    probs = torch.zeros_like(preds)                       # [B, B]
    ...
    if select is None:
        # 섞인 가중치 반영
        probs[rows, rows] = betas                         # diag ← beta
        probs[rows, perm] = 1 - betas                    # perm 위치 ← 1-beta
    else:
        # 섞인 애들만 가중치 반영
        sel_rows = rows[select]
        probs[sel_rows, sel_rows] = betas[select]
        probs[sel_rows, perm[select]] = 1 - betas[select]

    logp = F.log_softmax(preds, dim=1)
    loss = -(probs * logp).sum(dim=1).mean()
    return loss
...
def query_laion(text=None, emb=None, num=8, indice_name="laion5B-L-14", groundtruth=None, clip_extractor=None,
